    ...
    stderrRedirector(this)
{
    ...
    try
    {
        ...
        PySys_SetArgv(1, (wchar_t **)const_cast<char**>(argv));
...
void PythonInterpreter::runScriptFile(QString fileName)
{
    ...
    if (fp != NULL)
    {
        ...
        char *argv[1];
        argv[0] = const_cast<char*>(fname);
        PySys_SetArgv(1, (wchar_t **)argv); // python < 2.6.6 does not have PySys_SetArgvEx()

        PyRun_SimpleFileEx(fp, fname, 1); // 1 means close it for me
